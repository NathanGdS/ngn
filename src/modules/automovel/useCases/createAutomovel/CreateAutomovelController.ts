...
class CreateAutomovelController {
    async handle(request: Request, response: Response): Promise<Response> {
        try{
            const { plate, model, brand, color, year, renavam, customerId, typeId } = request.body;

            const createAutomovelUseCase = container.resolve(
                CreateAutomovelUseCase
            );

            const automovel = await createAutomovelUseCase.execute({ plate, model, brand, color, year, renavam, customerId, typeId });
